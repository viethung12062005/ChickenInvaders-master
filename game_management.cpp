    ...
    if (kill < NUMBER_OF_CHICKEN * 2) {
        for (int ck = 0; ck < p_chicken_list.size(); ck++) {
            ...
            if (p_chicken) {
                ...
                // Handle the case where p_chicken is null
                // Remove the null pointer from p_chicken_list
                p_chicken_list.erase(p_chicken_list.begin() + ck);
                ck--; // Adjust the index since an element was removed
                continue;
            }
